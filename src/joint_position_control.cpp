...
//     // Extract joint limits (reshape to n x 4 like your pose node)
//     std::vector<double> jl_lower, jl_upper, jl_vel, jl_eff;
//     get_parameter("joint_limits_lower",    jl_lower);
//     get_parameter("joint_limits_upper",    jl_upper);
//     get_parameter("joint_limits_velocity", jl_vel);
...
//     joint_limits_.assign(n_, std::array<double,4>{0,0,0,0});
//     for (int i = 0; i < n_; ++i) {
//       joint_limits_[i][0] = jl_lower[i];
//       joint_limits_[i][1] = jl_upper[i];
//       joint_limits_[i][2] = jl_vel[i];
...
//     // Validate per-joint screw params existence/shape (exactly like your pose node expects)
//     for (int j = 0; j < n_; ++j) {
...
//     // Print joint limits (optional: mirrors your pose code)
//     std::cout << "\n-- Left arm joint limits [ll, ul, vel, eff] [rad, rad, rad/s, Nm] -->\n";
//     for (int i = 0; i < n_; ++i) {
//       std::cout << "  " << joint_names_[i] << ": ["
//                 << joint_limits_[i][0] << ", "
...
//   void initROS()
//   {
//     // I/O topics match your current wiring
//     pub_ = create_publisher<sensor_msgs::msg::JointState>("/joint_states", rclcpp::SensorDataQoS());
//     sub_ = create_subscription<sensor_msgs::msg::JointState>(
//       "/openarm_left/joint_command", rclcpp::SensorDataQoS(), std::bind(&JointPositionControl::onCmd, this, _1));
...
//     if (msg.name.empty() || msg.position.size() < msg.name.size()) {
//       RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 2000,
//         "Bad /openarm_left/joint_commands (empty or positions < names). Ignoring.");
//       return;
//     }
//     // Update qcmd for any joints we model
//     const size_t m = msg.name.size();
//     for (size_t k = 0; k < m; ++k) {
//       auto it = name_to_idx_.find(msg.name[k]);
//       if (it == name_to_idx_.end()) continue; // ignore unknown (e.g., gripper)
...
//   void onTimer()
//   {
//     // First-order velocity response around a P position loop:
//     // qdot_cmd = Kp * (qcmd - q)
//     // qdd      = qdot_cmd - a * qdot
//     // integrate semi-implicit Euler
//     for (int i = 0; i < n_; ++i) {
//       const double qe  = qcmd_[i] - q_[i];
//       double qd_cmd   = Kp_ * qe;
//       // pre-saturate
//       double vl = (VEL_LIMIT_ == 0.0) ? 1e6 : VEL_LIMIT_;
//       if (qd_cmd >  vl) qd_cmd =  vl;
//       if (qd_cmd < -vl) qd_cmd = -vl;
//       const double qdd = qd_cmd - a_ * qd_[i];

//       qd_[i] += qdd * Ts_;
//       if (qd_[i] >  vl) qd_[i] =  vl;
//       if (qd_[i] < -vl) qd_[i] = -vl;
//       q_[i]  += qd_[i] * Ts_;
//     }
...
//     std::cout << "\nqe = [";
//     for (int i = 0; i < n_; ++i) {
//       std::cout << qcmd_[i] - q_[i];
//       if (i < n_-1) std::cout << ", ";
//     }
//     std::cout << "]\n" << std::flush;

//     // Publish measured joints with names from the screw YAML
//     sensor_msgs::msg::JointState js;
//     js.header.stamp = now();
//     js.name = joint_names_;
//     js.position = q_;
...
//     if (std::chrono::duration<double>(now_steady - t_last_log_).count() >= 2.0) {
//       t_last_log_ = now_steady;
//       double max_qe = 0.0, mse = 0.0;
//       for (int i = 0; i < n_; ++i) {
//         const double qe = qcmd_[i] - q_[i];
//         max_qe = std::max(max_qe, std::abs(qe));
//         mse  += qe * qe;
//       }
//       mse /= std::max(1, n_);
//       RCLCPP_INFO(get_logger(), "Track err: max=%.4f rad, rmse=%.4f rad", max_qe, std::sqrt(mse));
//     }
//   }
...
//   // joint limits packed to n x 4
//   std::vector<std::array<double,4>> joint_limits_;
...
//   const double Kp_;        // 20
//   const double a_;         // 40
//   const double VEL_LIMIT_; // 2 rad/s

//   // simulated states
//   std::vector<double> q_, qd_, qcmd_;
//   std::unordered_map<std::string, int> name_to_idx_;
